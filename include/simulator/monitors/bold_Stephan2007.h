...
namespace tvb {
    ...
    public:
        BoldStephan2007(double T, int N, double dt, const std::vector<int> &voi) {
            m_period = 0.5;
            this->config_for_sim(T, N, dt, voi);
        }

        void config_for_sim(double T, int N, double dt, const std::vector<int> &voi) {
            Monitor::config_for_sim(N, dt, voi);
            m_T = T;
            init();
        }

        StateTrack apply(const std::vector<double>& times, const std::vector<State>& states) override {
            throw("Not implemented!");
        }

        Matrixd apply(const Matrixd& signal) override;

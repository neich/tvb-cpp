...
from tvb.simulator.backend.cpp import CppBackend
...
class TestNbSim(BaseTestSim):
    ...
    #    return CppBackend().build_py_func(template, content, name='run_sim_tavg_chunked', print_source=print_source)
    ...
    def test_local_deterministic(self):
        ...
        sim = simulator.Simulator(
            ...
            coupling=coupling.SparseCoupling(),
            connectivity=self._random_network(),
            conduction_speed=np.inf,
            monitors=[
                monitors.Raw()
            ],
            integrator=integrators.EulerStochastic( # matching integrator
                dt=dt, 
        ...
        sim.current_state = np.zeros(sim.current_state.shape)
        sim.history.buffer.fill(0.0)
        (pdq_t, pdq_d), = CppBackend().run_sim(sim, simulation_length=10)
        (raw_t, raw_d), = sim.run(simulation_length=10)
    ...
    def test_local_deterministic_spatial(self):
        ...
        sim = simulator.Simulator(
            model=models.ReducedWongWangExcInh(
                eta=np.random.uniform(
                    models.ReducedWongWangExcInh.eta.domain.lo,
                    models.ReducedWongWangExcInh.eta.domain.hi,
                    size=N
                )
            ),
        ...
        (pdq_t, pdq_d), = CppBackend().run_sim(sim, simulation_length=1)
    ...
    def test_local_stochastic(self):
        ...
        (pdq_t, pdq_d), = CppBackend().run_sim(sim, nstep=200)
    ...
    def test_network_deterministic_nodelay(self):
        ...
        with numpy.errstate(all='ignore'):
            (pdq_t, pdq_d), = CppBackend().run_sim(sim, nstep=1, compatibility_mode=True)
            r_pdq, V_pdq = pdq_d[0,:,:,0] 
    ...
    def test_tavg_chunking(self):
        ...
        (pdq_t, pdq_d), = CppBackend().run_sim(sim, nstep=2000, chunksize=2000, compatibility_mode=True)
        r_pdq = pdq_d[:,0,:,0]
        V_pdq = pdq_d[:,1,:,0]
        (pdq_chu_t, pdq_chu_d), = CppBackend().run_sim(sim, nstep=2000, chunksize=500, compatibility_mode=True)
        r_pdq_chu = pdq_d[:,0,:,0]
        V_pdq_chu = pdq_d[:,1,:,0]
    ...
    def test_stim(self):
        ...
        (pdq_t, pdq_d), = CppBackend().run_sim(sim, simulation_length=simulation_length, compatibility_mode=True)
    ...
    def test_stim_chunked(self):
        ...
        (pdq_t, pdq_d), = CppBackend().run_sim(sim, simulation_length=simulation_length, compatibility_mode=True, chunksize=5000)

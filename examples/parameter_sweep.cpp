...
*/
    ...
    std::string out_prefix = cp.get_option<std::string>("out-file-prefix");
    ...
    int n_sim = 0;
    for (const tvb::ParameterSet& pset: param_combinations) {

        tvb::Float G = cp.init_from_parameters(model, pset);
        ...
        std::string file_prefix = string_format("%s_%d", out_prefix.c_str(), n_sim);
